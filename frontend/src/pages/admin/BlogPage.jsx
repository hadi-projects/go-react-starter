...
const BlogPage = () => {
    ...
    const handleExport = async (format) => {
        setIsExporting(true);
        try {
            const response = await exportBlog(format);
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            const filename = format === 'csv' ? 'blog.csv' : 'blog.xlsx';
    ...
    const tableActions = [
        ...(can('update-blog') ? [{ label: 'Edit', onClick: handleOpenModal }] : []),
        ...(can('delete-blog') ? [{ label: 'Delete', onClick: (row) => handleDelete(row.id), className: 'text-error' }] : []),
    ];
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-surface-on tracking-tight">Blog Management</h1>
                    <p className="text-sm text-surface-on-variant mt-1">Manage your blog instances.</p>
                </div>
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-4 pt-2">
                    ...
                    <TextField
                        label="Thumbnail"
                        name="thumbnail"
                        value={formData.thumbnail.toString()}
