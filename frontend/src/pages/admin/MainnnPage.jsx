...
const MainnnPage = () => {
    ...
    const columns = [
        ...
        { header: 'Makananan', accessor: 'makananan' },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await updateMainnn(editingId, formData);
                toast.success('Updated successfully');
            } else {
                await createMainnn(formData);
    ...
    const handleExport = async (format) => {
        setIsExporting(true);
        try {
            const response = await exportMainnn(format);
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            const filename = format === 'csv' ? 'mainnn.csv' : 'mainnn.xlsx';
    ...
    const tableActions = [
        ...(can('update-mainnn') ? [{ label: 'Edit', onClick: handleOpenModal }] : []),
        ...(can('delete-mainnn') ? [{ label: 'Delete', onClick: (row) => handleDelete(row.id), className: 'text-error' }] : []),
    ];
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-surface-on tracking-tight">Mainnn Management</h1>
                    <p className="text-sm text-surface-on-variant mt-1">Manage your mainnn instances.</p>
                </div>
            ...
            <Modal
                ...
                title={editingId ? 'Edit Mainnn' : 'Add Mainnn'}
            >
                <form onSubmit={handleSubmit} className="space-y-4 pt-2">
                    ...
                    <TextField
                        label="Makananan"
                        name="makananan"
                        value={formData.makananan.toString()}
                        onChange={(e) => setFormData({ ...formData, makananan: e.target.value })}
